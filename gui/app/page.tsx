...
export default function ChessPage() {
  ...
  // Opening name reported by engine
  const [openingName, setOpeningName] = useState<string>("");

  // Premove support
  const [premoveFrom, setPremoveFrom] = useState<string | null>(null);
  const [premoveTo, setPremoveTo] = useState<string | null>(null);
  const premoveFromRef = useRef<string | null>(null);
  const premoveToRef = useRef<string | null>(null);

  // Captured pieces display (letters according to chess.js: p n b r q k)
  const [whiteCaptured, setWhiteCaptured] = useState<string[]>([]); // pieces captured by White
  const [blackCaptured, setBlackCaptured] = useState<string[]>([]); // pieces captured by Black

  const scrollRef = useRef<HTMLDivElement>(null);

  // Helper: compute captured pieces arrays from UCI move list
  function computeCapturedFromUci(moves: string[]) {
    ...
    for (const m of moves) {
      ...
      const res = temp.move({ from, to, promotion: promo as any });
      if (res && res.captured) {
        // res.color is the color of the moving piece
        if (res.color === "w") {
          // white moved and captured a black piece
          wCap.push(res.captured as string);
        } else {
          bCap.push(res.captured as string);
        }
      }
    }
  ...
  useEffect(() => {
    if (
      !showSetup &&
      !gameOver &&
      (timeLeft.white === 0 || timeLeft.black === 0)
    ) {
      finishGame(true);
    }
    ...
    try {
      const res = await fetch("/api/engine", {
        ...
        body: JSON.stringify({
          action: "move",
          moves: currentUci,
          wtime: timeLeft.white * 1000,
          btime: timeLeft.black * 1000,
        }),
      });
      ...
      if (data.move) {
        ...
        if (result) {
          const newUci = [...currentUci, data.move];
          setBoardFen(gameRef.current.fen());
          setUciMoves((prev) => {
            const next = [...prev, data.move];
            setSelectedMoveIdx(next.length - 1);
            // recompute captured pieces from full history
            computeCapturedFromUci(next);
            return next;
          });
          ...
          if (gameRef.current.isGameOver()) {
            ...
            if (premoveFromRef.current && premoveToRef.current) {
              const pf = premoveFromRef.current;
              const pt = premoveToRef.current;
              premoveFromRef.current = null;
              premoveToRef.current = null;
              setPremoveFrom(null);
              setPremoveTo(null);
              setTimeout(() => {
                try {
                  const premoveResult = gameRef.current.move({
                    from: pf,
                    to: pt,
                    promotion: "q",
                  });
                  if (premoveResult) {
                    setBoardFen(gameRef.current.fen());
                    const uci = pf + pt + (premoveResult.promotion || "");
                    setUciMoves((prev) => {
                      const n = [...prev, uci];
                      setSelectedMoveIdx(n.length - 1);
                      return n;
                    });
                    setSanMoves([...gameRef.current.history()]);
                    if (gameRef.current.isGameOver()) {
                      ...
                      setTimeout(() => callEngine([...newUci, uci]), 200);
                    }
                  }
                } catch {
  ...
  const loadGame = async (filename: string) => {
    try {
      ...
      if (data.pgn) {
        ...
        // rebuild captured arrays from full game
        computeCapturedFromUci(uciHistory);
        setGameOver(true);
        setShowSetup(false);
  ...
  const goToMove = (idx: number | null) => {
    ...
    // Recompute captured pieces up to this move
    computeCapturedFromUci(uciMoves.slice(0, idx + 1));
  ...
  function tryMove(from: string, to: string): boolean {
    ...
    const isPlayerTurn =
      (turn === "w" && actualSide === "white") ||
      (turn === "b" && actualSide === "black");
    ...
    try {
      ...
      // Update captured pieces state if this move captured
      if (result.captured) {
        if (result.color === "w") {
          setWhiteCaptured((prev) => [...prev, result.captured as string]);
        } else {
          setBlackCaptured((prev) => [...prev, result.captured as string]);
        }
      }
      setUciMoves(newUci);
      ...
      if (gameRef.current.isGameOver()) {
        ...
        setTimeout(() => callEngine(newUci), 200);
      }
      return true;
    } catch {
      return false;
  ...
  const handleSquareClick = useCallback(
    ({ square }: any) => {
      // Premove: queue a move while engine thinks
      if (isThinking && !gameOver && !showSetup) {
        const turn = gameRef.current.turn();
        const isPlayerTurn =
          (turn === "w" && actualSide === "white") ||
          (turn === "b" && actualSide === "black");
        // It's the engine's turn right now — player can premove
        if (!isPlayerTurn) {
          if (premoveFrom === null) {
            // First click: select piece
            const piece = gameRef.current.get(square as any);
            if (piece && piece.color === (actualSide === "white" ? "w" : "b")) {
              premoveFromRef.current = square;
              setPremoveFrom(square);
              premoveToRef.current = null;
              setPremoveTo(null);
              setOptionSquares({
                [square]: { background: "rgba(100, 100, 255, 0.7)" },
              });
            }
          } else {
            // Second click: set destination
            premoveToRef.current = square;
            setPremoveTo(square);
            setOptionSquares({
              [premoveFrom]: { background: "rgba(100, 100, 255, 0.7)" },
              [square]: { background: "rgba(100, 100, 255, 0.4)" },
            });
          }
          return;
        }
      ...
      if (moveFrom === null) {
        ...
        moves.forEach((m) => {
          h[m.to] = {
            background: gameRef.current.get(m.to as any)
    ...
    <div className="min-h-screen bg-[#F0F0F0] text-black font-mono p-4 md:p-8 flex items-center justify-center">
      ...
      <AnimatePresence>
        {showSetup && (
          ...
          >
            ...
            >
              <h1 className="text-4xl font-black mb-8 border-b-[6px] border-black pb-4 uppercase">
                Match Setup
              </h1>
              <div className="space-y-6">
                {/* Player Name */}
                <div>
                  <label className="text-lg font-black mb-2 block uppercase tracking-widest">
                    Your Name
                  </label>
                  <input
                    type="text"
                    ...
                    {(["white", "random", "black"] as Side[]).map((s) => (
                      <button
                        ...
                        className={`py-3 border-[4px] border-black font-black uppercase transition-all ${
                          playerSide === s
                            ? "bg-[#FF80BF] shadow-none translate-x-1 translate-y-1"
                ...
                <div className="relative">
                  <div className="absolute inset-0 flex items-center">
                    <span className="w-full border-t-[4px] border-black"></span>
                  </div>
                  <div className="relative flex justify-center">
                    <span className="bg-white px-4 font-black uppercase text-xs">
                      or review
                    </span>
                  </div>
                </div>
                    ...
                    >
                      <div className="flex flex-col gap-1">
                        <div className="font-black text-xl uppercase tracking-tight">
                          {g.white} <span className="opacity-30">vs</span>{" "}
                          {g.black}
                        </div>
      ...
      <div className="max-w-7xl w-full grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
        <div className="lg:col-span-7 flex flex-col gap-5">
          {/* Opponent clock */}
          <div
            className={`p-4 border-[6px] border-black flex justify-between items-center shadow-[8px_8px_0px_#000] transition-colors ${
              gameRef.current.turn() === (actualSide === "white" ? "b" : "w")
                ? "bg-[#FF80BF]"
                : "bg-white"
            }`}
          >
            ...
            <span className="text-3xl font-black tabular-nums">
              {fmt(actualSide === "white" ? timeLeft.black : timeLeft.white)}
            </span>
          </div>
          ...
          <div className="flex gap-0">
            ...
            {showAnalysis && (
              <div
                className="w-8 border-[4px] border-black border-r-0 flex flex-col overflow-hidden"
                style={{ minHeight: "100%" }}
              >
                <div
                  className="bg-[#1a1a1a] transition-all duration-500"
                  style={{
                    height: `${actualSide === "white" ? 100 - evalBarPct : evalBarPct}%`,
                  }}
                >
                  {evalBarPct < 50 && (
                    <div className="text-white text-[9px] font-black text-center pt-1">
                      ...
                      analysisData?.[selectedMoveIdx]?.isMate
                        ? "M"
                        : Math.abs(
                            Math.round(((50 - evalBarPct) / 5) * 10) / 10,
                          )}
                    </div>
                  )}
                </div>
                  ...
                  {evalBarPct >= 50 && (
                    <div className="text-black text-[9px] font-black text-center pt-1">
                      ...
                      analysisData?.[selectedMoveIdx]?.isMate
                        ? "M"
                        : Math.abs(
                            Math.round(((evalBarPct - 50) / 5) * 10) / 10,
                          )}
                    </div>
                  )}
                </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <button
              ...
              disabled={
                selectedMoveIdx === uciMoves.length - 1 || uciMoves.length === 0
              }
              className="py-3 bg-white border-[4px] border-black shadow-[6px_6px_0px_#000] hover:-translate-x-1 hover:-translate-y-1 active:translate-x-1 active:translate-y-1 active:shadow-none transition-all flex items-center justify-center gap-2 font-black uppercase disabled:opacity-50"
            >
          ...
          <div
            className={`p-4 border-[6px] border-black flex justify-between items-center shadow-[8px_8px_0px_#000] transition-colors ${
              gameRef.current.turn() === (actualSide === "white" ? "w" : "b")
                ? "bg-[#00E676]"
                : "bg-white"
            }`}
          >
            ...
            <span className="text-3xl font-black tabular-nums">
              {fmt(actualSide === "white" ? timeLeft.white : timeLeft.black)}
            </span>
          </div>
        </div>
        ...
        <div className="lg:col-span-5 flex flex-col gap-6 self-stretch">
          {/* Opening Name */}
          {openingName && !showSetup && (
            <div className="bg-[#1a1a1a] border-[6px] border-black p-4 shadow-[10px_10px_0px_#FFD600]">
              <h2 className="text-[10px] font-black uppercase tracking-[0.3em] mb-1 text-[#FFD600] border-b border-[#FFD600]/30 pb-1">
                Opening
              </h2>
          ...
          {/* Captured pieces */}
          <div className="bg-white border-[6px] border-black p-3 shadow-[8px_8px_0px_#000]">
            <div className="text-[10px] font-black uppercase tracking-widest mb-2">
              Captured Pieces
            </div>
            <div className="flex flex-col gap-2">
              <div className="flex items-center gap-3">
                <div className="w-24 text-[11px] font-black uppercase opacity-70">
                  By White
                </div>
                <div className="flex items-center gap-1">
                  {whiteCaptured.length === 0 ? (
                    ...
                    whiteCaptured.map((p, i) => (
                      <span key={i} className="text-xl" title={p}>
                        {p === "p"
                          ? "♟"
                          : p === "n"
                            ? "♞"
          ...
          <div className="bg-[#FFD600] border-[6px] border-black p-6 shadow-[10px_10px_0px_#000]">
            <h2 className="text-[10px] font-black uppercase tracking-[0.3em] mb-2 border-b-[3px] border-black pb-1">
              Status
            </h2>
            <p className="text-2xl font-black italic uppercase">{status}</p>
            {isThinking && (
              <div className="mt-2 h-2 bg-black/20 overflow-hidden">
          ...
          {showAnalysis && analysisData && (
            <div className="bg-white border-[6px] border-black p-6 shadow-[10px_10px_0px_#000]">
              <h2 className="text-[10px] font-black uppercase tracking-[0.3em] mb-4 border-b-[3px] border-black pb-1">
                Performance
              </h2>
              <div className="grid grid-cols-2 gap-4">
                <div className="border-[3px] border-black p-3">
                  ...
                  <div className="text-sm font-black text-[#43A047]">
                    Est. {whitePerf}
                  </div>
                </div>
                <div className="border-[3px] border-black p-3 bg-black text-white">
                  ...
                  <div className="text-sm font-black text-[#26C6DA]">
                    Est. {blackPerf}
                  </div>
                </div>
              </div>
          ...
          <div className="flex-1 bg-white border-[6px] border-black p-6 shadow-[10px_10px_0px_#000] flex flex-col min-h-[300px] max-h-[500px]">
            ...
            <div
              ref={scrollRef}
              className="flex-1 overflow-y-auto text-sm font-bold"
            >
              {sanMoves.length === 0 ? (
                <div className="h-full flex items-center justify-center text-black/10 text-3xl uppercase font-black">
                  Waiting
                </div>
              ) : (
                <table className="w-full">
                  <tbody>
                    {Array.from({ length: Math.ceil(sanMoves.length / 2) }).map(
                      (_, i) => {
                        const wIdx = i * 2;
                        const bIdx = i * 2 + 1;
                        ...
                        return (
                          <tr key={i} className="border-b border-black/10">
                            <td className="py-1.5 pr-2 w-8 text-[10px] opacity-30 font-black">
                              {i + 1}.
                            </td>
                            <td
                              className={`py-1.5 pr-1 cursor-pointer hover:bg-black/5 ${selectedMoveIdx === wIdx ? "bg-[#FFD600]/30" : ""}`}
                              onClick={() => goToMove(wIdx)}
                            >
                              <span>{sanMoves[wIdx]}</span>
                              {wA && (
                                <span
                                  className="ml-1 text-[10px] font-black px-1 py-0.5 inline-block rounded-sm"
                                  style={{
                                    backgroundColor:
                                      CLASS_COLORS[
                                        wA.classification as MoveClass
                                      ],
                                    color: "#fff",
                                  }}
                                >
                            ...
                            <td
                              className={`py-1.5 cursor-pointer hover:bg-black/5 ${selectedMoveIdx === bIdx ? "bg-[#FFD600]/30" : ""}`}
                              onClick={() => {
                                if (sanMoves[bIdx]) goToMove(bIdx);
                              }}
                            >
                              {sanMoves[bIdx] && (
                                <>
                                  ...
                                  {bA && (
                                    <span
                                      ...
                                      style={{
                                        backgroundColor:
                                          CLASS_COLORS[
                                            bA.classification as MoveClass
                                          ],
                                        color: "#fff",
                                      }}
                                    >
