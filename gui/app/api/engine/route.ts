...
const ENGINE_PATH = path.join(process.cwd(), '..', 'target', 'release', 'prometheus');
...
export async function POST(req: NextRequest) {
  ...
  if (body.action === 'save-analysis') {
    ...
    fs.writeFileSync(path.join(ANALYZED_GAMES_DIR, analyzedFilename), JSON.stringify(analysisData, null, 2));
  ...
  // List games
  if (body.action === 'list') {
    if (!fs.existsSync(PLAYED_GAMES_DIR)) return NextResponse.json({ games: [] });
    const files = fs.readdirSync(PLAYED_GAMES_DIR).filter(f => f.endsWith('.pgn'));
    ...
    return NextResponse.json({ games: games.sort((a, b) => b.filename.localeCompare(a.filename)) });
  }
